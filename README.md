...
From the beginning, each philosopher does three (mutual exclusive) actions: eat, sleep or think: they wait the availability of the forks on their left and right so they can eat, with a full belly they sleep and then they think, so the cycle can start again.

The catch is that a philosophers need two forks to eat, so they have to take turn for eating: as a consequence at most n/2 philosophers can eat at the same time.
...
## Parameters:
The program takes some parameteres to set the environments *(see #Disclaimer#2 for parameters limitations)*;
